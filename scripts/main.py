...
def train_process(rank, train_dataloader, val_dataloader, model, loss, optimizer, lr_schedule):
    ...
    writer = SummaryWriter('/root/storage/models/tb/')
    for epoch in range(to_restore['epoch'], training_params[mode]['num_epochs']):
        
        
        start_time = time.time()  # time, bong
        ...
        writer.add_scalar('acc1', acc1, epoch + 1)
        writer.add_scalar('acc5', acc5, epoch + 1)
        writer.add_scalar('time', time.time() - start_time, epoch + 1)  # bong, time
